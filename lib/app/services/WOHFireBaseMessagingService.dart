...
class WOHFireBaseMessagingService extends GetxService {
          ...
          builder: (_){
            return AlertDialog(
              content: Container(
                ...
                child: Text('Allow your device to receive notifications from Hubkilo in the device settings', style: Get.textTheme.displayLarge!.merge(TextStyle(fontSize: 15, color: buttonColor))),
              ),
            );
          });
    }
  ...
  void _newMessageNotificationBackground(RemoteMessage message) {
    if (message.data['messageId'] != null) {
      Get.toNamed(WOHRoutes.CHAT, arguments: new Message([], id: message.data['messageId']));
    }
  }
  ...
  void _bookingNotification(RemoteMessage message) {
    ...
    Get.showSnackbar(WOHUi.notificationSnackBar(
